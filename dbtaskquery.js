...
// TO INSERT INTO COLLECTION NAMED PRODUCT
...
db.getCollection('product').find({$or:[{"product_price":{$lte:400}},{"product_price":{$gte:600}}]}).pretty();
...
db.product.aggregate([{ $project : { _id : 0, product_material : 1, product_name : 1} }]).pretty()
...
db.product.find({}).sort({product_price: 1 })
db.product.deleteMany({product_price:{$eq:47}})
db.product.find().pretty();